...
def parse_metadata(parsed_publications: list[dict]) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        {
            "pub_id": [article["pub_id"] for article in parsed_publications],
...
def parse_metadata_ocr_files(ocr_publications: list[dict]) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        {
            "pub_id": [article["pub_id"] for article in ocr_publications],
            "title": np.nan,
            "date": np.nan,
            "doi": np.nan,
...
def reconstruct_publication_year(
    parsed_metadata: pd.DataFrame, hot_fixes: dict[str, str]
) -> pd.DataFrame:
    """ """
    ...
    parsed_metadata["reconstructed_date"] = parsed_metadata[
        "reconstructed_date"
    ].replace(hot_fixes)
...
if __name__ == "__main__":
    ...
    ### parse good files
    publications_parsed = read_jsonl(os.path.join(OUTDIR, "publications_parsed.ndjson"))
    ...
    meta_parsed.to_csv(
        os.path.join(OUTDIR, "meta_publications_parsed.csv"), index=False
    )
